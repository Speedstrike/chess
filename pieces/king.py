    ...
    def get_available_moves(self, board):
        if self.last_moves_check != board.move_count or board.move_count == 0:
            ...
            if current_y - 1 > -1:
                ...
                if self.can_move(up) or self.can_capture(up) and not board.is_in_check(up, board.white_turn):
            ...
            if current_x - 1 > -1:
                ...
                if self.can_move(left) or self.can_capture(left) and not board.is_in_check(left, board.white_turn):
                    self.available_moves.append(left)

            # Right
            if current_x + 1 < 8:
                right = board.get_square((current_x + 1, current_y))
                if self.can_move(right) or self.can_capture(left) and not board.is_in_check(right, board.white_turn):
                    self.available_moves.append(right)

            # Top Left Diagonal
            if current_x - 1 > -1 and current_y - 1 > -1:
                top_left = board.get_square((current_x - 1, current_y - 1))
                if self.can_move(top_left) or self.can_capture(top_left) and not board.is_in_check(top_left, board.white_turn):
                    self.available_moves.append(top_left)

            # Top Right Diagonal
            if current_x + 1 < 8 and current_y - 1 > -1:
                top_right = board.get_square((current_x + 1, current_y - 1))
                if self.can_move(top_right) or self.can_capture(top_right) and not board.is_in_check(top_right, board.white_turn):
                    self.available_moves.append(top_right)

            # Bottom Left Diagonal
            if current_x - 1 > -1 and current_y + 1 < 8:
                bottom_left = board.get_square((current_x - 1, current_y + 1))
                if self.can_move(bottom_left) or self.can_capture(bottom_left) and not board.is_in_check(top_left, board.white_turn):
                    self.available_moves.append(bottom_left)

            # Bottom Right Diagonal
            if current_x + 1 < 8 and current_y + 1 < 8:
                bottom_right = board.get_square((current_x + 1, current_y + 1))
                if self.can_move(bottom_right) or self.can_capture(bottom_right) and not board.is_in_check(bottom_right, board.white_turn):
                    self.available_moves.append(bottom_right)
